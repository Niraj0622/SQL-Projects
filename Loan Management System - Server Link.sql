...
CREATE TABLE customer_income_status (Loan_id VARCHAR(10) PRIMARY KEY, Customer_id VARCHAR(10), Applicant_income INT, Co_Applicant_income INT, Property_Area VARCHAR(10), 
Loan_Status VARCHAR(10), Loan_Amount INT);
DESCRIBE Customer_income_status;
ALTER TABLE Customer_income_status MODIFY Customer_id varchar(10) PRIMARY KEY;
ALTER TABLE Customer_income_status MODIFY Loan_id TEXT , MODIFY Property_area varchar(25), MODIFY Loan_status varchar(5), MODIFY Loan_amount varchar (25);


-- Sheet 1 task

-- Grading Customer with their Income
SELECT * FROM Customer_income_status;
SELECT *, 
IF (Applicant_income >15000, 'Grade_A', IF(Applicant_income >9000, 'Grade_B', IF(Applicant_income >5000, concat('Middle Class Customer'), concat('Low Class')))) AS Class
FROM Customer_income_status;

CREATE TABLE Customer_income_status1 AS SELECT *, 
IF (Applicant_income >15000, 'Grade_A', IF(Applicant_income >9000, 'Grade_B', IF(Applicant_income >5000, concat('Middle Class Customer'), concat('Low Class')))) AS Class
FROM Customer_income_status;
...
-- Monthly Interest Percentage
    ...
    END
    Else 7
    END  AS Monthly_interest_Percentage FROM Customer_income_status1;
    
    
    SELECT*,CASE
        WHEN Applicant_income < 5000 AND Property_area = 'rural' THEN Concat('3%')
        WHEN  Applicant_income < 5000 AND Property_area = 'semirural' THEN concat('3.5%')
        WHEN  Applicant_income < 5000 AND Property_area = 'urban' THEN concat('5%')
        WHEN  Applicant_income < 5000 AND Property_area = 'semiurban' THEN concat('2.25%')
        ELSE  concat('7%')
    END AS Monthly_interest_Percentage FROM Customer_income_status1;
    
    CREATE TABLE Customer_income_status2 AS SELECT*,CASE
        WHEN Applicant_income < 5000 AND Property_area = 'rural' THEN Concat('3%')
        WHEN  Applicant_income < 5000 AND Property_area = 'semirural' THEN concat('3.5%')
        WHEN  Applicant_income < 5000 AND Property_area = 'urban' THEN concat('5%')
        WHEN  Applicant_income < 5000 AND Property_area = 'semiurban' THEN concat('2.25%')
...
-- Calculate Monthly interest and Annual Interest amount
-- Monthly Interest SELECT*,
 CASE
        WHEN  Property_area = 'rural' THEN Floor(Loan_amount*0.03)
        WHEN Property_area = 'semirural' THEN floor(Loan_amount *0.035)
        WHEN  Property_area = 'urban' THEN floor(Loan_amount*0.05)
        WHEN  Property_area = 'semiurban' THEN floor(Loan_amount *0.025)
        ELSE  floor(Loan_amount *0.07)
    END AS Monthly_interest_Amount FROM Customer_income_status2;

    CREATE TABLE Customer_income_status3 as 
 SELECT*,
 CASE
        WHEN  Property_area = 'rural' THEN Floor(Loan_amount*0.03)
        WHEN Property_area = 'semirural' THEN floor(Loan_amount *0.035)
...
-- Annual Interest
SELECT*, Monthly_interest_Amount*12 AS Annual_interest_Amount FROM Customer_income_status3 ;
     
CREATE TABLE Customer_interest_analysis AS SELECT*, Monthly_interest_Amount*12 AS Annual_interest_Amount FROM Customer_income_status3 ;

SELECT * FROM Customer_interest_analysis;

DROP TABLE Customer_income_status1;
DROP TABLE Customer_income_status2;
DROP TABLE Customer_income_status3;


-- Task 2
SELECT * FROM loan_Status;
DESCRIBE loan_status;
ALTER TABLE loan_status MODIFY customer_id varchar(12) PRIMARY KEY;
...
SELECT * FROM Loan_status1;
SELECT * FROM Cibil_score_status;
DROP TRIGGER Loan_verify;
DROP TRIGGER Cibil_score_verify;
DROP TABLE Loan_status1;
DROP TABLE Cibil_score_status;
...
DELIMITER ##
CREATE TRIGGER cibil_score_verify
AFTER INSERT ON Loan_status1 FOR EACH ROW
BEGIN
    IF NEW.cibil_score > 900 THEN
       INSERT INTO Cibil_score_status(Customer_id , Cibil_status )
        VALUES (new.Customer_id, Concat('High CIBIL score'));
        
       ELSE IF NEW.cibil_score > 750 THEN
        INSERT INTO Cibil_score_status(Customer_id , Cibil_status )
        VALUES (new.Customer_id, Concat('No penalty'));
        
        ELSE IF NEW.cibil_score > 250 THEN
         INSERT INTO Cibil_score_status(Customer_id , Cibil_status )
        VALUES (new.Customer_id, Concat('Penalty customer'));
        
        ELSE IF NEW.cibil_score <= 0 THEN
       INSERT INTO Cibil_score_status(Customer_id , Cibil_status )
        VALUES (new.Customer_id, Concat('Reject customer'));
...
Insert Into Loan_status1 VALUES
('LP001002',	'IP43001',	'Loan still processing',	360,	303),
('LP001003',	'IP43002',	128,	360,	920),
('LP001005',	'IP43003',	66,	360,	606),
('LP001006',	'IP43004',	120,	360,	851),
('LP001008',	'IP43005',	141,	360,	420),
...
('LP001098',	'IP43034',	114,	360,	667),
('LP001100',	'IP43035',	320,	360,	889),
('LP001106',	'IP43036',	'Loan still processing',	360,	460),
('LP001109',	'IP43037',	100,	0,	413),
('LP001112',	'IP43038',	144,	360,	139),
('LP001114',	'IP43039',	184,	360,	812),
('LP001116',	'IP43040',	110,	360,	71),
...
('LP001206',	'IP43062',	99,	360,	803),
('LP001207',	'IP43063',	165,	180,	889),
('LP001213',	'IP43064',	'Loan still processing',	360,	291),
('LP001222',	'IP43065',	116,	360,	924),
('LP001225',	'IP43066',	258,	360,	261),
('LP001228',	'IP43067',	126,	180,	339),
('LP001233',	'IP43068',	312,	360,	551),
...
('LP001264',	'IP43080',	130,	360,	951),
('LP001265',	'IP43081',	111,	360,	431),
('LP001266',	'IP43082',	'Loan still processing',	360,	499),
('LP001267',	'IP43083',	167,	360,	309),
('LP001273',	'IP43084',	265,	360,	393),
('LP001275',	'IP43085',	50,	240,	933),
('LP001279',	'IP43086',	136,	360,	720),
...
('LP001322',	'IP43094',	122,	360,	148),
('LP001325',	'IP43095',	25,	120,	680),
('LP001326',	'IP43096',	'Loan still processing',	360,	394),
('LP001327',	'IP43097',	137,	360,	558),
('LP001333',	'IP43098',	50,	360,	994),
('LP001334',	'IP43099',	115,	180,	632),
('LP001343',	'IP43100',	131,	360,	213),
('LP001345',	'IP43101',	133,	180,	759),
('LP001349',	'IP43102',	151,	360,	263),
('LP001350',	'IP43103',	'Loan still processing',	360,	722),
('LP001356',	'IP43104',	'Loan still processing',	360,	142),
('LP001357',	'IP43105',	160,	360,	200),
('LP001367',	'IP43106',	100,	360,	157),
('LP001369',	'IP43107',	225,	360,	636),
('LP001370',	'IP43108',	120,	360,	550),
...
('LP001387',	'IP43112',	139,	360,	680),
('LP001391',	'IP43113',	152,	0,	140),
('LP001392',	'IP43114',	'Loan still processing',	360,	979),
('LP001398',	'IP43115',	118,	360,	278),
('LP001401',	'IP43116',	185,	180,	478),
('LP001404',	'IP43117',	154,	360,	919),
('LP001405',	'IP43118',	85,	360,	570),
...
('LP001443',	'IP43126',	93,	360,	403),
('LP001448',	'IP43127',	370,	360,	249),
('LP001449',	'IP43128',	'Loan still processing',	360,	261),
('LP001451',	'IP43129',	160,	180,	929),
('LP001465',	'IP43130',	182,	360,	454),
('LP001469',	'IP43131',	650,	480,	840),
('LP001473',	'IP43132',	74,	360,	208),
...
('LP001674',	'IP43201',	90,	360,	420),
('LP001677',	'IP43202',	166,	360,	289),
('LP001682',	'IP43203',	'Loan still processing',	180,	613),
('LP001688',	'IP43204',	135,	360,	947),
('LP001691',	'IP43205',	124,	360,	642),
('LP001692',	'IP43206',	120,	360,	638),
('LP001693',	'IP43207',	80,	360,	303),
...
('LP001915',	'IP43283',	78,	180,	818),
('LP001917',	'IP43284',	54,	360,	592),
('LP001922',	'IP43285',	'Loan still processing',	360,	686),
('LP001924',	'IP43286',	89,	360,	760),
('LP001925',	'IP43287',	99,	300,	497),
('LP001926',	'IP43288',	120,	360,	55),
('LP001931',	'IP43289',	115,	360,	447),
...
('LP001977',	'IP43304',	96,	360,	53),
('LP001978',	'IP43305',	140,	360,	98),
('LP001990',	'IP43306',	'Loan still processing',	360,	969),
('LP001993',	'IP43307',	135,	360,	268),
('LP001994',	'IP43308',	104,	360,	147),
('LP001996',	'IP43309',	480,	360,	362),
('LP001998',	'IP43310',	185,	360,	361),
...
('LP002051',	'IP43321',	115,	360,	654),
('LP002053',	'IP43322',	124,	360,	953),
('LP002054',	'IP43323',	'Loan still processing',	360,	53),
('LP002055',	'IP43324',	132,	360,	381),
('LP002065',	'IP43325',	300,	360,	819),
('LP002067',	'IP43326',	376,	360,	373),
('LP002068',	'IP43327',	130,	360,	324),
...
('LP002110',	'IP43337',	160,	360,	389),
('LP002112',	'IP43338',	176,	360,	979),
('LP002113',	'IP43339',	'Loan still processing',	360,	236),
('LP002114',	'IP43340',	71,	360,	895),
('LP002115',	'IP43341',	173,	360,	365),
('LP002116',	'IP43342',	46,	360,	258),
('LP002119',	'IP43343',	158,	360,	735),
...
('LP002237',	'IP43386',	113,	180,	52),
('LP002239',	'IP43387',	132,	360,	337),
('LP002243',	'IP43388',	'Loan still processing',	360,	611),
('LP002244',	'IP43389',	136,	360,	884),
('LP002250',	'IP43390',	125,	360,	320),
('LP002255',	'IP43391',	185,	360,	897),
('LP002262',	'IP43392',	275,	360,	882),
...
('LP002387',	'IP43434',	143,	360,	953),
('LP002390',	'IP43435',	100,	360,	539),
('LP002393',	'IP43436',	'Loan still processing',	240,	345),
('LP002398',	'IP43437',	50,	360,	961),
('LP002401',	'IP43438',	'Loan still processing',	360,	535),
('LP002403',	'IP43439',	187,	360,	272),
('LP002407',	'IP43440',	138,	360,	865),
('LP002408',	'IP43441',	360,	360,	0),
('LP002409',	'IP43442',	180,	360,	631),
...
('LP002530',	'IP43478',	132,	360,	431),
('LP002531',	'IP43479',	86,	360,	416),
('LP002533',	'IP43480',	'Loan still processing',	360,	910),
('LP002534',	'IP43481',	154,	360,	981),
('LP002536',	'IP43482',	113,	360,	950),
('LP002537',	'IP43483',	128,	360,	226),
('LP002541',	'IP43484',	234,	360,	74),
...
('LP002692',	'IP43523',	100,	360,	768),
('LP002693',	'IP43524',	480,	360,	271),
('LP002697',	'IP43525',	'Loan still processing',	360,	211),
('LP002699',	'IP43526',	400,	360,	818),
('LP002705',	'IP43527',	110,	360,	397),
('LP002706',	'IP43528',	161,	360,	534),
('LP002714',	'IP43529',	94,	360,	773),
...
('LP002776',	'IP43549',	103,	360,	258),
('LP002777',	'IP43550',	110,	360,	376),
('LP002778',	'IP43551',	'Loan still processing',	360,	160),
('LP002784',	'IP43552',	'Loan still processing',	360,	80),
('LP002785',	'IP43553',	158,	360,	437),
('LP002788',	'IP43554',	181,	360,	63),
('LP002789',	'IP43555',	132,	180,	867),
('LP002792',	'IP43556',	26,	360,	632),
...
('LP002958',	'IP43604',	172,	360,	863),
('LP002959',	'IP43605',	496,	360,	234),
('LP002960',	'IP43606',	'Loan still processing',	180,	944),
('LP002961',	'IP43607',	173,	360,	201),
('LP002964',	'IP43608',	157,	360,	439),
('LP002974',	'IP43609',	108,	360,	478),
('LP002978',	'IP43610',	71,	360,	982),
...
SELECT l.loan_id,l.customer_id,l.loan_amount,l.loan_amount_term,l.cibil_score,c.cibil_status
FROM loan_status1 l join cibil_score_status c On l.customer_id = c.customer_id;

CREATE TABLE Loan_cibil_status_details as SELECT l.loan_id,l.customer_id,l.loan_amount,l.loan_amount_term,l.cibil_score,c.cibil_status
FROM loan_status1 l join cibil_score_status c On l.customer_id = c.customer_id;

SELECT * FROM loan_cibil_status;


-- Delete reject and loan still processing customers
DELETE FROM Loan_cibil_status
WHERE cibil_status = 'Reject customer' OR loan_amount = 'Loan still processing';
SELECT * FROM Loan_cibil_status;

-- Alter table to change loan_amount to INT
ALTER TABLE Loan_cibil_status
MODIFY COLUMN loan_amount INT;
Describe Loan_cibil_status;


-- Task 3
SELECT * FROM Customer_info;
DESCRIBE Customer_info;
ALTER TABLE Customer_info MODIFY Customer_id varchar(12) PRIMARY KEY;
...
SELECT * FROM Region;
DESCRIBE customer_income_status;
ALTER TABLE customer_income_status MODIFY Loan_id VARCHAR(25) PRIMARY KEY;
DESCRIBE Country_state;
ALTER TABLE Country_state MODIFY Loan_id VARCHAR(25) PRIMARY KEY;
DESCRIBE loan_status;
ALTER TABlE loan_status DROP PRIMARY KEY;
ALTER TABLE loan_status MODIFY Loan_id VARCHAR(25) PRIMARY KEY;
DESCRIBE customer_info;
ALTER TABlE customer_info DROP PRIMARY KEY;
ALTER TABLE customer_info MODIFY Loan_id VARCHAR(25) PRIMARY KEY;
DESCRIBE Region;
ALTER TABLE loan_data_set MODIFY REGION_id varchar(12) PRIMARY KEY;

-- Joining All 5 Tables
SELECT d.loan_id,d.Customer_name,d.Gender,d.age,d.married,d.education,d.self_employed,d.region_id,m.customer_id,m.applicant_income,
m.co_applicant_income,m.property_area,m.loan_status,m.loan_amount, c.postal_code,c.segment,c.state,l.loan_amount_term,l.cibil_score,r.region
FROM customer_info d JOIN customer_income_status m On d.loan_id = m.loan_id Join Country_state c On c.Region_id = d.Region_id 
JOIN loan_status l On l.loan_id = d.loan_id Join Region r On r.region_id = d.region_id;
...
SELECT l.loan_id,l.Customer_id,l.loan_amount,L.loan_amount_term,l.cibil_score,c.Applicant_income
FROM Loan_status l
INNER JOIN customer_income_status c ON L.Loan_id = c.Loan_id order by Cibil_score DESC LIMIT 0,10;
...
SELECT S.Customer_id,s.Customer_name,s.Region_id,c.gender,c.age,s.postal_code,s.segment,s.state
FROM Country_state s Inner Join customer_info c On s.Customer_id = c.Customer_id WHERE Segment In ('Home office', 'Corporate') ;
